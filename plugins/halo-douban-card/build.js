...
module.exports = {
   ...
   */
  usingComponents: {
    'halo-douban-card': '../halo-douban-card/halo-douban-card'
  },
  /**
  ...
  handler (file, platform) {
    if (file.isBuffer()) {
      ...
      if (file.path.includes('parser.js')) {
        content = content.replace(/trustTags\s*:\s*makeMap\('/, 'trustTags: makeMap(\'douban,').replace(/voidTags\s*:\s*makeMap\('/, 'voidTags: makeMap(\'douban,')
      }
