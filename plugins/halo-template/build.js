...
module.exports = {
   ...
   */
  usingComponents: {
    'halo-component-name': '../halo-component-name/halo-component-name'
  },
  /**
  ...
  handler (file, platform) {
    if (file.isBuffer()) {
      ...
      if (file.path.includes('parser.js')) {
        ...
        content = content.replace(/trustTags\s*:\s*makeMap\('/, 'trustTags: makeMap(\'componentName,').replace(/voidTags\s*:\s*makeMap\('/, 'voidTags: makeMap(\'componentName,')
      }
