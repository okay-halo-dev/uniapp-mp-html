 ...
 */
HaloPluginName.prototype.onUpdate = function (content, config) {
  config.ignoreTags.xxx = true // 移除 xxx 标签
  // todo:对 html 内容进行预处理并返回修改，没有修改则不需要返回
 ...
 */
HaloPluginName.prototype.onParse = function (node, parser) {
  // 处理文本标签
  if (node.type === 'text') {
    // node.text 文本内容
    ...
    if (node.name === 'halo-component-name') {
 ...
 */
HaloPluginName.prototype.onLoad = function () {
  // 可以获取媒体 context 对象等
}
 ...
 */
HaloPluginName.prototype.onDetached = function () {
  // 可以释放一些必要的资源（计时器等）
}

module.exports = HaloPluginName
